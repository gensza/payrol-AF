    ...
    if ($module == 'tunjangan_jabatan' AND $act == 'tambah') {
        ...
        if (mysql_num_rows($cek) > 0) {
            echo"<script language='javascript'>
                        alert('Tunjangan yang anda masukkan sudah ada. Periksa kembali data tunjangan karyawan yang sudah ada !');
        ...
        $besar_tunjangan = trim_number($_POST['besar_tunjangan']);
        $query = mysql_query("UPDATE tunjangan_jabatan SET nip = '$_POST[nip]', id_jenis_tunjangan = '$_POST[id_jenis_tunjangan]', besar_tunjangan = '$besar_tunjangan' WHERE id_tunjangan_jabatan = '$_POST[id]'");
        if ($query) {
