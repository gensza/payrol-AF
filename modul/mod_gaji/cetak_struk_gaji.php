...
if ($jumlah_jam_kehadiran > 160) {
	$jumlah_jam_lembur = $jumlah_jam_kehadiran - 160; //MENGHITUNG JUMLAH JAM LEMBUR
	$jumlah_jam_potongan = 0; // MENGHITUNG JUMLAH JAM POTONGAN
	$gaji_lembur = ($gaji_jam = $datak['gapok'] / 160) * ($jumlah_jam_lembur); // MENGHITUNG GAJI LEMBUR
	...
	$jumlah_jam_potongan = 160 - ($jumlah_kehadiran * 8); // MENGHITUNG JUMLAH JAM POTONGAN
	$gaji_lembur = ($datak['gapok'] / 160) * ($jumlah_jam_lembur); // MENGHITUNG GAJI LEMBUR
	$besar_potongan = ($datak['gapok'] / 160) * ($jumlah_jam_potongan); // MENGHITUNG BESAR POTONGAN
}
...
if ($jumlah_absensi < 1) {
	...
	$pdf->Cell(0, 6, 'YAYASAN ASA FATIHA INDONESIA', 0, 1, 'C');
	$pdf->Ln(-2);
	...
	$pdf->Cell(0, 4, '(Jumlah jam kehadiran normal : 160 Jam)', '', 1, 'L');
	$pdf->ln();
	$pdf->ln();
	//PERHITUNGAN GAJI
	$pdf->setX(20);
	...
	$pdf->Cell(0, 4, ': Rp. ', '', 0, 'L');
	$pdf->setX(74);
	$pdf->Cell(30, 4, indo_number_without_rp(($datak['gapok'] / 160) * ($jlh_acuti * 8)), '', 1, 'R');
	$pdf->setX(20);
	$pdf->Cell(0, 4, '- Sakit (' . $jlh_asakit . ' Hari)', '0', 0, 'L');
	$pdf->setX(70);
	$pdf->Cell(0, 4, ': Rp. ', '', 0, 'L');
	$pdf->setX(74);
	$pdf->Cell(30, 4, indo_number_without_rp(($datak['gapok'] / 160) * ($jlh_asakit * 8)), '', 1, 'R');
	$pdf->setX(20);
	$pdf->Cell(0, 4, '- Ijin (' . $jlh_aijin . ' Hari)', '0', 0, 'L');
	$pdf->setX(70);
	$pdf->Cell(0, 4, ': Rp. ', '', 0, 'L');
	$pdf->setX(74);
	$pdf->Cell(30, 4, indo_number_without_rp(($datak['gapok'] / 160) * ($jlh_aijin * 8)), '', 1, 'R');
	$pdf->setX(20);
	$pdf->Cell(0, 4, '- T.Keterangan (' . $jlh_aalfa . ' Hari)', '0', 0, 'L');
	$pdf->setX(70);
	$pdf->Cell(0, 4, ': Rp. ', '', 0, 'L');
	$pdf->setX(74);
	$pdf->Cell(30, 4, indo_number_without_rp(($datak['gapok'] / 160) * ($jlh_aalfa * 8)), '', 1, 'R');
	$pdf->setX(74);
	$pdf->Cell(30, 4, '', 'T', 1, 'R');
	$pdf->setX(40);
	$pdf->SetFont('times', 'B', '10');
